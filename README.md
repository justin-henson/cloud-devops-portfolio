# Cloud/DevOps Portfolio — review path + demos

This repo is the quickest way to review my work for Cloud / DevOps / Infrastructure roles . It links to 6 projects in the order I'd walk through them in an interview — each one builds on the last.

## If you only have 5 minutes

1) **AWS Terraform baseline** (infra demo: create → verify → destroy)
   Repo: https://github.com/justin-henson/cloud-baseline-aws
...
2) **CI/CD pipeline** (automated deployment with safety gates)
   Repo: https://github.com/justin-henson/cicd-pipeline
...
3) **Kubernetes baseline** (EKS with security hardening)
   Repo: https://github.com/justin-henson/k8s-baseline-eks
...
4) **Observability stack** (monitoring + alerting + SLOs)
   Repo: https://github.com/justin-henson/observability-stack
...
5) **Ops / SRE runbooks** (operate + recover + improve)
   Repo: https://github.com/justin-henson/ops-runbooks
...
|------|----------------------|------------------------|
| [`cloud-baseline-aws`](https://github.com/justin-henson/cloud-baseline-aws) | Terraform AWS baseline, private EC2 via SSM, ALB ingress | Run the **Proof** steps in the README |
| [`cicd-pipeline`](https://github.com/justin-henson/cicd-pipeline) | GitHub Actions CI/CD with drift detection | Check the GitHub Actions tab for passing workflows |
| [`k8s-baseline-eks`](https://github.com/justin-henson/k8s-baseline-eks) | EKS with security hardening, IRSA, Pod Security Standards | Review `k8s/` manifests and Terraform config |
| [`observability-stack`](https://github.com/justin-henson/observability-stack) | CloudWatch + Prometheus monitoring, SLOs, alert-driven runbooks | Check `terraform/`, `prometheus/`, `slo/` directories |
| [`ops-runbooks`](https://github.com/justin-henson/ops-runbooks) | Ops maturity: incident/change/DR templates | Open incident response and postmortem templates |

## Role fit

- Target roles: Cloud / DevOps / Infrastructure / SRE
- Based in Texas (CT) — flexible working hours to overlap with team timezone
- Open to remote opportunities worldwide; open to sponsorship and relocation
